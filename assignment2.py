...
class Grid:
    ...
    def nillson(self):
        ...
        if self.grid[4] != 0: ## +1 if center tile is not empty
            score += 1
        for i, tile in enumerate(self.grid): ## +2 for every element out of place, clockwise with respect to GOAL
            if tile != 0 and i != 4 and (self.grid[i], self.grid[clockwise_elements[i]]) != (
                    Grid.GOAL[i], Grid.GOAL[clockwise_elements[i]]):
                score += 2
        score *= 3
        score += self.manhatten_distance() ## add the manhatten distance of a grid to the score
    ...
    def search(self, visited, LIMIT):
        ## Base Cases:
        ## the goal is reached
        ## the search limit is exceeded
        if self.isGoal():
            path = []
            visited.append(self.grid)
            self.findPath(path)
            return path
        if LIMIT >= 1: ## If method is still within range
            visited.append(self.grid)
            child_states = self.next_states() ## Recursive call
            LIMIT -= 1
            for child in child_states:
                if child.grid not in visited:
                    child.parent = self
    ...
    def bfs(self):
        ...
        while not states.empty():  ## While the priority queue is not empty, take the first element
            ...
            if state.grid not in visited:
                ...
                current_time = time_ns()  ## When the search goes on longer than 60s, method times out.
                if current_time - start_time >= 60000000000:
                    return False
    ...
    def ucs(self):
        ...
        while not states.empty():
            ...
            if state.grid not in visited:
                ...
                current_time = time_ns()  ## When the search goes on longer than 60s, the method times out.
                if current_time - start_time >= 60000000000:
                    return False
                ...
                for child in child_states:
                    if child.grid not in visited:
                        child.parent = state    ## For each child not visited, place child in priority queue
                        states.put((self.cost(child), child))
        return False
    ## A* search
    def A(self):
        start_time = time_ns()  ## start time
            ...
            if state.grid not in visited:
                ...
                for child in child_states:  ## For each child, set child.parent to current state
                    ...
                    if child.grid not in visited:  ## For each child not visited, place child in priority queue
                        ## total cost: sum of nillson score and child depth
                        child_total_cost = child.depth + child.nillson()
    ...
    def dfs(self, LIMIT):
        ...
        ## If search is successful, return length of visited nodes and the path to goal
        if not path:
            return False
        return len(visited), path   ## Otherwise, return false
...
if __name__ == '__main__':  ## Driver
    while 1:  ## Sentinel loop
        ...
        print("--------------------------------")
        print("1: Depth-First Search (slow, low chance of success)")
        print("2: Uniform Cost Search (very slow, very low chance of success)")
        ...
        if user_input == 1:
            while limit > 30:
                try:
                    limit = int(input("Please enter a depth limit no greater than 30 for Depth-First Search: "))
                except ValueError:
                    limit = 31
        print("--------------------------------")
        print("Searching...")
        ...
        ## If no solution to the algorithm is found in the case of dfs
        ...
        ## Print the path taken, as well as the number of nodes visited.
            ...
            print(f"Time taken: {total_time / 1000000000} seconds")
        print("--------------------------------")

        user_input = ""

        while user_input != "y" and user_input != "n":
            user_input = input("New Puzzle? (Y/N):  ")
            user_input.lower()
